...
function init() {
  ...
  function animate() {
    ...
    const markerX = (tlWidth * mouseX) / width;
    ...
    // Update info overlay
    const currentTime = progress * trackDuration;
    info.innerHTML = `
      <div>AGGRESSION: ${aggression.toFixed(3)}</div>
      <div>PROGRESS: ${(progress * 100).toFixed(1)}%</div>
      <div>TIME: ${currentTime.toFixed(1)}s / ${trackDuration.toFixed(1)}s</div>
      <div style="margin-top: 8px; opacity: 0.6;">Move mouse to scrub</div>
      <div style="margin-top: 8px; opacity: 0.4;">SVG Filter Effects</div>
